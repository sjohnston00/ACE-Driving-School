...
namespace ACE_Driving_School.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            //user can only delete their account if the have no upcoming lessons or bookings
            return View();
        }
        ...
        {
            User user = context.Users.Find(model.User_Id);

            user.PhoneNumber = model.New_PhoneNumber;
            context.SaveChanges();
            return RedirectToAction("Index", "Manage");
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var AddingToRole = await UserManager.AddToRoleAsync(student.Id, "Student");
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                Student student = (Student)await UserManager.FindByEmailAsync(user.Email);
                var AddingToRole = await UserManager.AddToRoleAsync(student.Id, "Student");
                if (result.Succeeded && AddingToRole.Succeeded)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        var message = new IdentityMessage
                        {
                            Destination = user.Email,
                            Subject = "Please Confirm Email",
                            Body = $"Thank you for signing up to our app through Facebook. " +
                            $"\nYou can only make booking and start learning to drive when you have confirmed your Email and Phone Number." +
                            $"\n Please confirm your account by clicking this link: \n{callbackUrl}",
                        };
